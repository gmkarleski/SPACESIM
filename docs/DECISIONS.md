...
### Phase 2 Track B design foundation — procgen philosophy + BodyState schema shape + schema-vs-code principle (commit 059)

**Date:** 2026-05-25
**Question:** Three load-bearing decisions to lock as the design foundation for Phase 2 Track B (per-planet procgen): (D1) the procgen design philosophy at the conceptual level — what kind of procgen system this is; (D2) the BodyState schema shape — composition vs inheritance, dictionary vs named-nullable, the boundary between what's locked at 059 and what's per-sub-object design work for subsequent commits; (D3) the schema-vs-code split — what's in the type system vs what's in implementation logic.
**Decision:** Three locked decisions.

**D1 — Procgen design philosophy: physics-not-gating, coherence-not-content.** Full statement in `docs/world/PROCGEN_DESIGN.md` (new file added at this commit). Summary: physics simulation drives outcomes (not narrative gates); coherence comes from constraint propagation through the pipeline (not from hand-authored content). Hand-tuned home-system bodies are hand-tuned PARAMETERS that obey the same coherence machinery as procgen-generated bodies — parameter values are chosen but the constraint propagation is identical. Difficulty is in engineering problems, not arbitrary unlocks. Bodies are interesting because they're physically interesting, not because a designer tagged some as interesting.

**D2 — BodyState schema shape: composition-all-the-way-down, sealed POCO, named-nullable sub-objects, no inheritance, no feature dictionary. Shape locked; specific sub-object decomposition NOT locked.** Full structure in `docs/world/PROCGEN_DESIGN.md`. Summary: single sealed C# class containing ten physics-universal flat shared fields (BodyId, Seed, Name, Position, ParentBodyId, Mass, Radius, SoiRadius, RotationRate, AxialTilt) plus a structure of nullable domain sub-objects representing body subsystems plus a structure of named-nullable feature subsystems plus a sub-object for detection signatures. Body-type asymmetry (star vs planet vs moon vs asteroid) expressed by sub-object presence/absence, not class hierarchy. **The specific sub-object decomposition — what sub-objects exist, their names, their granularity (e.g., one combined surface-state sub-object vs separate geology and terrain sub-objects; one volcanism sub-object with a fluid parameter vs separate volcanism and cryovolcanism sub-objects), whether candidate items belong as domain sub-objects or as feature subsystems — is per-commit design work for subsequent commits (060+).** Sub-object internal contents (what fields live inside each sub-object) also NOT locked at this commit.

**D3 — Schema-vs-code principle: schema reflects state, code reflects logic.** Activation rules (when does a sub-object populate), anti-correlation rules (volcanism + rings dampened), cascade order (which sub-objects depend on which), validation (low-geology bodies don't produce volcanism) all live in procgen pipeline code. Schema only captures the resulting state. Same principle drives VesselAuthoritativeState ↔ VesselDesign separation in NETCODE_CONTRACT §2.1 / §2.5.

**Alternatives rejected:**

- **Class inheritance hierarchy `Body → StellarBody | PlanetaryBody` (D2).** Forces a permanent type distinction in the type system. Costs: polymorphic JSON serialization ceremony, ReferenceBody MonoBehaviour wrapping complexity (would need `[SerializeReference]` polymorphic handling or split MonoBehaviour per subclass), no clean handling for edge cases (brown dwarfs, pulsars, hybrid bodies). Gains: compile-time safety against accidentally reading `star.Atmosphere`. Rejected — Foundation/ codebase has zero inheritance hierarchies; existing precedent is composition + nullable references for variants. Compile-time safety gain is small (most code is type-aware at call site, not generically iterating mixed bodies); composition handles edge cases gracefully.
- **Nullable-variant pattern (single Body class with `StellarState?` and `PlanetaryState?` sub-objects parallel to VesselAuthoritativeState's mode-state pattern) (D2).** Mirrors existing precedent shape exactly. Rejected because the VesselAuthoritativeState mode-state pattern is for TRANSITIONAL variants (a vessel transitions PhysX ↔ Kepler ↔ Cruise); body-type asymmetry is PERMANENT (stars don't become planets). Borrowing the pattern misrepresents the semantic — transitional-variant vocabulary applied to permanent-type distinction. Composition-all-the-way-down at the sub-object level is the honest pattern.
- **Feature dictionary `Dictionary<FeatureKind, FeatureState>` for special features (D2).** Flexible, extensible. Rejected because polymorphic FeatureState subtypes would re-introduce inheritance at the feature level for the same reasons we reject it at the body level. Named-nullable feature fields are consistent with the rest of the schema; growth is additive (new field per new feature, no central dictionary to maintain). CONSTRAINTS §6 line 1522's mention of "extensible special features dictionary" is documentation that drifted ahead of the design we're now locking; future commit reconciles.
- **Lock specific sub-object decomposition at 059 alongside the shape (D2).** Drafted into earlier versions of this commit's PROCGEN_DESIGN.md content — sub-objects named (a stellar-emission sub-object, an orbital-dynamics sub-object, atmosphere, hydrosphere, magnetosphere, geology, terrain, biomes, resources, plus feature subsystems for rings, auroras, volcanism, cryovolcanism, extreme weather, etc.). Rejected on review because those names were inherited from CONSTRAINTS §6 and STATUS.md text without examination — they're plausible candidates but contain real undecided design questions (geology vs terrain as one sub-object or two; extreme weather as feature or atmospheric subset; volcanism + cryovolcanism as one sub-object with a fluid parameter or two distinct sub-objects). Locking inherited names would be the same pattern we just identified at the pipeline level (§6/STATUS.md parallel-pipeline situation). The shape (composition + named-nullable + no inheritance + no dictionary) is the load-bearing decision; specific decomposition is per-commit design work that 060+ does.
- **Schema enforces activation rules / anti-correlation rules / cascade order (D3).** Putting validation in the type system (e.g., "a body with high mass can't have low geology" as a struct invariant) would catch bad combinations at compile/load time. Rejected because the rules are pipeline-side computation, not type-system invariants — they require numerical thresholds, statistical sampling, and grammar evaluation that don't fit in a POCO. Validation lives where the computation lives.
- **Lock the §6 vs STATUS.md pipeline reconciliation at 059 (out of scope per design).** Both documents enumerate "14 stages" of per-body procgen but with substantively different stage progressions (parameter-focused vs rendering-focused). Reconciliation is real work — could be unification, could be "two layers of one pipeline," could be retire-one. Deferring lets 059 land the design foundation without forcing a pipeline-choice that the schema doesn't actually depend on. Future commit reconciles. Documented explicitly in PROCGEN_DESIGN.md "Existing pipeline language" section so future readers see the deferred-reconciliation status.
- **Defer all three Ds to subsequent commits as separate one-D commits (D1+D2+D3 packaging).** Each D could be its own commit. Rejected because they're tightly coupled — the design philosophy (D1) motivates the schema shape (D2); the schema shape (D2) only makes sense alongside the schema-vs-code split (D3); the schema-vs-code split (D3) is a corollary of the design philosophy (D1). Packaging them together preserves the coherent story.

**Rationale (smallest decisive design foundation commit; lock the load-bearing structural shape but not the per-sub-object decomposition):** The three locked decisions form the design foundation that subsequent stage-specific commits build against. D1 names the kind of procgen system this is; D2 names the structural shape state takes — and explicitly does not lock the specific sub-object decomposition, which is per-commit design work; D3 names the type-system / logic split. Together they're the scaffolding for sub-object design (which happens per-sub-object in 060+) and pipeline cascade structure (which lands at pipeline reconciliation, a future commit). The shape doesn't depend on which pipeline (§6 or STATUS.md or their reconciliation) ends up canonical — schema is upstream of pipeline choice. The shape also doesn't depend on which specific sub-objects end up in the schema — the structural pattern (named-nullable, no inheritance, no dictionary) holds regardless of decomposition.

**Implication:** Subsequent commits (060+) design individual sub-objects. Natural next: the sub-object(s) that will hold stellar state (continuing 058's Stage 1 thread), then orbital state (most bodies have it; orbital mechanics well-understood from existing Vessels work), then iteratively per sub-object. Each per-sub-object commit decides naming, granularity, and internal contents for that one sub-object. Pipeline cascade structure reconciliation (the §6 vs STATUS.md question) is its own future commit when enough sub-objects are designed to make the cascade tractable. Track A (vessel construction) parallel-track per §9 phase-exit criteria.

**Locked in:** commit 059.
