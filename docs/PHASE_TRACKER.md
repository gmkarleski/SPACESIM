...
**Phase 1 system-list work (per the "Systems by phase → Phase 1 — Foundations" section) can now proceed cleanly.** The two Phase 0 carryover items are resolved; reference frame hierarchy + patched conics landed at commit 044; event prediction priority queue (the item that unblocks time-warp gating) landed at commit 045; SOI crossing predictor landed at commit 046. The remaining Phase 1 work is additional event predictors (atmospheric entry + surface impact at commit 047), continuous time-warp UI and rate-scaling controls at commit 048 (the queue lookup is wired but the player-facing warp controls aren't), save/load format, vessel container data structure refinements as gameplay systems land.
